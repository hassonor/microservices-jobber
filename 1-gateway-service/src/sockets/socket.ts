...
import { IMessageDocument, IOrderDocument, IOrderNotifcation, winstonLogger } from '@ohjobber/shared';
import { gatewayConfig } from '@gateway/config';

const log: Logger = winstonLogger(`${gatewayConfig.ELASTIC_SEARCH_URL}`, 'gatewaySocket', 'debug');
...
export class SocketIOAppHandler {
  ...
  private chatSocketServiceIOConnections(): void {
    chatSocketClient = io(`${gatewayConfig.MESSAGE_BASE_URL}`, {
  ...
  private orderSocketServiceIOConnections(): void {
    orderSocketClient = io(`${gatewayConfig.ORDER_BASE_URL}`, {
      transports: ['websocket', 'polling'],
      secure: true
    });
