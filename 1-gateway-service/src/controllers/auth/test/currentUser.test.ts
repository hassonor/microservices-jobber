...
describe('CurrentUser', () => {
  ...
  describe('getLoggedInUsers method', () => {
    it('should return correct response', async () => {
      ...
      jest.spyOn(GatewayCache.prototype, 'getLoggedInUsersFromCache').mockResolvedValue(['hasson_test', 'user_2']);
      jest.spyOn(socketServer.socketIO, 'emit');

      await CurrentUserController.prototype.getLoggedInUsers(req, res);
      expect(socketServer.socketIO.emit).toHaveBeenCalledWith('online', ['hasson_test', 'user_2']);
      expect(res.status).toHaveBeenCalledWith(200);
  ...
  describe('removeLoggedInUser method', () => {
    it('should return correct response', async () => {
      ...
      const res: Response = authMockResponse();
      jest.spyOn(GatewayCache.prototype, 'removeLoggedInUserFromCache').mockResolvedValue(['hasson_test']);
      jest.spyOn(socketServer.socketIO, 'emit');

      await CurrentUserController.prototype.removeLoggedInUser(req, res);
      expect(socketServer.socketIO.emit).toHaveBeenCalledWith('online', ['hasson_test']);
      expect(res.status).toHaveBeenCalledWith(200);
