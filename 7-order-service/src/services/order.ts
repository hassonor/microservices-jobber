import { IDeliveredWork, IExtendedDelivery, IOrderDocument, IOrderMessage, IReviewMessageDetails } from '@ohjobber/shared';
import { OrderModel } from '@order/models/order';
import { publishDirectMessage } from '@order/queues/producers/order';
import { orderChannel } from '@order/server';
import { sendNotification } from '@order/services/notification';
import { lowerCase } from 'lodash';
import { orderConfig } from '@order/config';

export const getOrderByOrderId = async (orderId: string): Promise<IOrderDocument> => {
  const order: IOrderDocument = (await OrderModel.findOne({ orderId })) as IOrderDocument;
  return order;
};

export const getOrdersBySellerId = async (sellerId: string): Promise<IOrderDocument[]> => {
  const orders: IOrderDocument[] = (await OrderModel.find({ sellerId })) as IOrderDocument[];
  return orders;
};

export const getOrdersByBuyerId = async (buyerId: string): Promise<IOrderDocument[]> => {
  const orders: IOrderDocument[] = (await OrderModel.find({ buyerId })) as IOrderDocument[];
  return orders;
};
...
export const createOrder = async (data: IOrderDocument): Promise<IOrderDocument> => {
  ...
  const emailMessageDetails: IOrderMessage = {
    ...
    orderUrl: `${orderConfig.CLIENT_URL}/orders/${data.orderId}/activities`,
...
export const approveOrder = async (orderId: string, data: IOrderMessage): Promise<IOrderDocument> => {
  ...
  const messageDetails: IOrderMessage = {
    ...
    totalEarnings: data.totalEarnings, // this is the price the seller earned for latest order delivered
...
export const sellerDeliverOrder = async (orderId: string, delivered: boolean, deliveredWork: IDeliveredWork): Promise<IOrderDocument> => {
  ...
  if (order) {
    const messageDetails: IOrderMessage = {
      ...
      orderUrl: `${orderConfig.CLIENT_URL}/orders/${orderId}/activities`,
