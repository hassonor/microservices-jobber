...
import { IConversationDocument, IMessageDetails, IMessageDocument } from '@ohjobber/shared';
import { MessageModel } from '@chat/models/message';
import { lowerCase } from 'lodash';
import { publishDirectMessage } from '@chat/queues/chatProducer';
...
const addMessage = async (data: IMessageDocument): Promise<IMessageDocument> => {
  ...
  if (data.hasOffer) {
    const emailMessageDeails: IMessageDetails = {
    ...
    await publishDirectMessage(
      ...
      'order-email',
      JSON.stringify(emailMessageDeails),
...
const markMultipleMessagesAsRead = async (reciever: string, sender: string, messageId: string): Promise<IMessageDocument> => {
